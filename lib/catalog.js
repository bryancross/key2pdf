...
catalog.prototype.updateCatalog = function(job)
{

        /*
        job.github.repos.getContent({

                owner: job.config.kbOwner,
                repo: job.config.kbRepo,
...
function updateJSONCatalog(job) {
        ...
        job.files.forEach(function (keynote) {
            ...
            if (!index && index != 0) {
                ...
                catEntry.url = "https://" + (job.config.targetHost != 'api.github.com' ? job.config.targetHost : job.config.targetHost.substring(4)) + "/" + job.config.kbOwner + "/" + job.config.kbRepo + "/blob/" + job.config.targetBranch + "/" + keynote.path
                //catEntry.PDFurl = catEntry.url + ".pdf"
                catEntry.PDFurl = keynote.GDriveURL;
            ...
            {
                job.catalog[index].PDFurl = keynote.GDriveURL;
        ...
        {
            job.github.repos.createFile({owner:job.config.kbOwner,repo:job.config.kbRepo,path:"keynote-catalog.json",message:job.config.commitMsg,content:catContent})
                .then(function(err, res)
                {
        ...
        {
            job.github.repos.updateFile({owner:job.config.kbOwner, repo:job.config.kbRepo, path:"keynote-catalog.json",message:job.config.commitMsg,content:catContent,sha:job.catalog.sha})
                .then(function(err,res)
                {
                    updateMarkdownCatalog(job);
                })
...
function uploadCatalog(job)
    {
        ...
        job.github.repos.getContent({owner:job.config.kbOwner,
            repo:job.config.kbRepo,
            path:"keynote-catalog.md"}
        ).then(function(err,res){
            logger.log("Found Markdown Catalog" + err.sha, job);

            job.github.repos.updateFile({owner:job.config.kbOwner, repo:job.config.kbRepo, path:"keynote-catalog.md",message:job.config.commitMsg,content:b64content,sha:err.sha})
                .then(function(err,res)
                {
            ...
            job.github.repos.createFile({owner:job.config.kbOwner,repo:job.config.kbRepo,path:"keynote-catalog.md",message:job.config.commitMsg,content:b64content})
                .then(function(err,res)
                {
